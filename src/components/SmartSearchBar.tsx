...
export function SmartSearchBar({ 
    ...
    return (
        <div className="w-full max-w-[700px] mx-auto relative" ref={containerRef}>
            ...
            <div
                className={`flex items-center bg-white dark:bg-[#1e1e2e] border-[1.5px] border-slate-200 dark:border-[#374151] rounded-full shadow-[0_4px_20px_rgba(0,0,0,0.08)] transition-all duration-200 ease-in-out focus-within:border-[#6366f1] focus-within:shadow-[0_4px_25px_rgba(99,102,241,0.15)] ${isFocused ? 'border-[#6366f1]' : ''}`}
            >
                {/* Left Icon Area */}
                <div className="pl-[20px] pr-[12px] flex items-center justify-center shrink-0">
                    {isSearching ? (
                        <Loader2 className="w-[20px] h-[20px] text-[#6366f1] animate-spin" />
                    ) : (
                        <Search className="w-[20px] h-[20px] text-[#6366f1]" />
                    )}
                </div>
                    ...
                    {query && (
                        <>
                            ...
                            <button
                                ...
                                className="bg-[#6366f1] hover:bg-[#4f46e5] text-white font-semibold py-[8px] px-[20px] rounded-full transition-colors ml-1 shadow-sm"
                            >
                                Search
                            </button>
                        </>
            ...
            {parsedFilters && !isSearching && query && (
                <div className="flex flex-wrap items-center gap-[8px] mt-4 px-2 animate-in fade-in slide-in-from-top-1">
                    {parsedFilters.country && (
                        <span className="bg-[#eef2ff] text-[#4f46e5] dark:bg-[#312e81] dark:text-[#a5b4fc] rounded-full px-[12px] py-[4px] text-[13px] font-medium inline-flex items-center gap-1.5 border border-[#c7d2fe]/50 dark:border-indigo-400/20">
