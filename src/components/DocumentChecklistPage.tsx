...
export function DocumentChecklistPage({ onNavigate }: { onNavigate: (page: string, params?: any) => void }) {
    ...
    // Force dark theme - always dark mode
    const isDark = true;
    ...
    return (
        <div className={`min-h-screen flex font-sans bg-gray-950 text-white`}>
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                <header className="no-print px-8 py-4 flex items-center justify-between border-b z-10 backdrop-blur-md bg-gray-900/80 border-gray-800">
                    <div>
                        <h1 className="text-2xl font-black tracking-tight text-white">Smart Document Tracker</h1>
                        <p className="text-sm font-medium text-gray-400">Interactive application document roadmap.</p>
                    </div>

                    <div className="flex items-center gap-4">
                        <ThemeToggle />
                        <div className="flex items-center gap-2 px-4 py-2 rounded-2xl border bg-green-500/10 border-green-500/20">
                ...
                <main className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-10">
                    <div className="max-w-4xl mx-auto space-y-8">
                        <div className="p-6 sm:p-8 rounded-3xl border flex flex-col sm:flex-row items-center gap-6 sm:gap-8 bg-gray-800 border-gray-700 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.3)]">
                            ...
                            <div className="flex-1 text-center sm:text-left">
                                <h2 className="text-2xl font-black mb-2 tracking-tight text-white">Application Readiness</h2>
                                <p className="font-medium mb-4 text-gray-400">
                            ...
                            {DOCUMENT_CATEGORIES.map(category => {
                                ...
                                return (
                                    <div key={category.id} className="space-y-4">
                                        <div className="sticky top-0 z-10 flex items-center justify-between p-4 rounded-2xl border backdrop-blur-xl bg-gray-900/90 border-gray-800">
                                            ...
                                            <div className={`text-xs font-bold px-3 py-1 rounded-full border ${isCatComplete ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-gray-800 text-gray-400 border-gray-700'}`}>
                                            ...
                                            {category.items.map(item => {
                                                ...
                                                return (
                                                    <div 
                                                        ...
                                                        className={`group flex items-center gap-4 p-4 rounded-2xl border cursor-pointer transition-all duration-300 bg-gray-800 border-gray-700 hover:bg-gray-700 ${isChecked ? 'border-l-4 border-l-green-500 bg-green-500/5' : 'border-l-4 border-l-transparent'}`}
                                                    >
                                                        <div className="shrink-0 flex items-center justify-center transition-colors">
                                                            {isChecked ? (
                                                                ...
                                                                <Circle size={28} className="text-gray-600 group-hover:text-gray-500" />
                                                            )}
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                                                <h4 className={`font-bold truncate ${isChecked ? 'text-green-400' : 'text-white'}`}>
                                                                    {item.name}
                                                                </h4>
                                                                <span className="shrink-0 text-xs font-bold px-3 py-1 rounded-full w-fit bg-gray-700 text-gray-300">
                                                                    {item.prepTime}
                                                                </span>
                                                            </div>
