...
export default function TeacherProfilePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "sans-serif" }}>
      ...
      {/* ── HERO ─────────────────────────────────────────────── */}
      <div style={{ background: "linear-gradient(135deg,#163065 0%,#312e81 55%,#4338ca 100%)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: -40, left: -40, width: 260, height: 260, borderRadius: "50%", background: "radial-gradient(circle,rgba(244,196,78,0.12) 0%,transparent 70%)", pointerEvents: "none" }} />

        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px 0", position: "relative" }}>
          {/* Back button */}
          <button onClick={() => navigate("/courses")}
            style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "#c7d2fe", fontSize: 13, fontWeight: 700, background: "rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 12, padding: "9px 18px", cursor: "pointer", marginBottom: 32, transition: "all 0.2s" }}
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
                {specs.map((spec: string, i: number) => (
                  <span key={i} style={{ background: `${TEST_COLORS[spec] || "#f4c44e"}25`, border: `1px solid ${TEST_COLORS[spec] || "#f4c44e"}50`, color: "#fff", fontSize: 12, fontWeight: 800, padding: "4px 12px", borderRadius: 100 }}>
                    {spec}
                  </span>
                ))}
              </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 24px 72px", display: "flex", flexDirection: "column", gap: 28 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #e2e8f0", padding: "28px 32px", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
          <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1e293b", margin: "0 0 16px", display: "flex", alignItems: "center", gap: 8 }}>
            <GraduationCap size={20} color="#d4a017" /> About {teacher.name}
          </h2>
        ...
        {/* Courses */}
        <div>
          <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1e293b", margin: "0 0 18px", display: "flex", alignItems: "center", gap: 8 }}>
            <Zap size={20} color="#d4a017" /> Available Courses
            <span style={{ background: "#f5f3ff", color: "#d4a017", fontSize: 13, fontWeight: 800, padding: "2px 10px", borderRadius: 100 }}>{teacher.courses?.length || 0}</span>
          </h2>
          ...
          {!teacher.courses?.length ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 18 }}>
              {teacher.courses.map((course: any) => {
                const color = TEST_COLORS[course.test_type] || "#f4c44e";
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ padding: "18px 20px", flex: 1, display: "flex", flexDirection: "column" }}>
                      ...
                      <h3 style={{ fontSize: 15, fontWeight: 800, color: "#1e293b", margin: "0 0 8px", lineHeight: 1.3 }}>{course.title}</h3>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid #f1f5f9", paddingTop: 12 }}>
                        ...
                        <span style={{ display: "flex", alignItems: "center", gap: 4, color: "#d4a017", fontSize: 12, fontWeight: 700 }}>View <ChevronRight size={14} /></span>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #e2e8f0", overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
          <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1e293b", margin: 0 }}>Student Reviews</h2>
          ...
          <div style={{ padding: "0 0 0" }}>
            <TeacherReviews
              ...
              isEnrolled={teacher.courses?.some((c: any) => c.enrolled) ?? false}
              canReview={
                userRole === "student" &&
                (teacher.courses?.some((c: any) => c.enrolled) ?? false)
              }
...
function TeacherSkeleton() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9" }}>
      <div style={{ background: "linear-gradient(135deg,#163065,#312e81)", padding: "28px 24px 60px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ width: 130, height: 36, borderRadius: 10, background: "rgba(0,0,0,0.08)", marginBottom: 32 }} />
          <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
            <div style={{ width: 96, height: 96, borderRadius: 24, background: "rgba(0,0,0,0.1)" }} />
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ width: "45%", height: 28, borderRadius: 8, background: "rgba(0,0,0,0.1)" }} />
              <div style={{ width: "30%", height: 18, borderRadius: 8, background: "rgba(0,0,0,0.08)" }} />
              <div style={{ width: "60%", height: 14, borderRadius: 8, background: "rgba(0,0,0,0.05)" }} />
            </div>
          </div>
        </div>
      </div>
